...
struct PersistentSegTree {
  ...
  PersistentSegTree(int n, int q) {
    ...
    roots.push_back(
        0); // 0-th index in roots can serve as a dummy or 0th version
    node_count = 1;
  }
  ...
  int build(vector<int> &a, int lx, int rx) {
    if (rx - lx == 1) {
      if (lx < a.size())
        return create_node(a[lx], 0, 0);
      else
        return create_node(0, 0, 0);
    }
  ...
  void update(int i, int val, int prev_version) {
    roots.push_back(update(i, val, roots[prev_version], 0, size));
  }
  ll query(int l, int r, int version) {
    return query(l, r, roots[version], 0, size).sum;
  }
};